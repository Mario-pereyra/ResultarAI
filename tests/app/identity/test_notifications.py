...
def test_notification_types_registry_status():
...
def test_emit_notification_success():
...
def test_emit_notification_rejected_unregistered():
    """Emisión rechazada para un tipo no registrado."""
    user_id = make_user("notif-unreg")

    with get_db_session() as db:
        with pytest.raises(ValueError, match="Tipo de notificación no registrado"):
            emit_notification(
                db=db,
...
def test_emit_notification_rejected_role_validation():
    """Emisión rechazada si el destinatario no cumple el rol esperado."""
    user_id = make_user("non-admin-user", role="tecnico")

    with get_db_session() as db:
        with pytest.raises(ValueError, match="no cumple con los requisitos del rol"):
            emit_notification(
                db=db,
                notification_type="quota_release_requested",
                recipient_id=user_id,
