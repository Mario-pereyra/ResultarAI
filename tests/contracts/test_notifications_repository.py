...
def test_notification_repository_lifecycle():
    ...
    with get_db_session() as db:
        list_a = repo.list_notifications(db, recipient_id=user_a.id, status="unread", limit=1, offset=0)
        assert len(list_a) == 1
        # The list is desc, so the second one should be first
        assert list_a[0].type == "quota_release_resolved"

        list_all = repo.list_notifications(db, recipient_id=user_a.id, status="all", limit=10, offset=0)
...
def test_notification_repository_retention():
    repo = PostgresNotificationRepository()
    ...
    with get_db_session() as db:
        ...
        notif.created_at = datetime.datetime.now(datetime.UTC).replace(tzinfo=None) - timedelta(days=5)
        db.commit()
